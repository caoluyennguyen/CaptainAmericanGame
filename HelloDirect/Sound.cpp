...
Sound* Sound::getInstance(HWND hwnd)
{
	if (instance == nullptr)
...
Sound::Sound(HWND hWnd)
{
	...
	if (FAILED(result))
	{
		DebugOut(L"[ERROR] Can not create device");
	}
	...
	if (FAILED(result))
	{
		DebugOut(L"[ERROR] Can not create primaryBuffer");
	}
	volume = 100.0f;
	isMute = false;
}
...
void Sound::loadSound(char* fileName, std::string name)
{
	...
	if (error != 0)
	{
		DebugOut(L"[ERROR] Can not load: %s", fileName);
		return;
	}
	...
	{
		DebugOut(L"[ERROR] file format does not support: %s", fileName);
	}
	...
	if (FAILED(result))
	{
		DebugOut(L"[ERROR] Can not create secondaryBuffer ");
		return;
	}
	...
	if (error != 0)
	{
		DebugOut(L"[ERROR] Can not close file ");
	}
...
{
	isMute = true;
	Sound::getInstance(this->hWnd)->stop();
}
